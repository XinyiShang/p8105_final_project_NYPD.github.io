...
## Are crime rates associated with poverty, income, and educational level? 
...
### Apply Lasso Regression to fine tune the prediction model and remove predictors that are not influential enough. 

```{r}
# Finding the best lambda
cv_model <- cv.glmnet(data.matrix(ses_df[, c('pop_pov_pct', 'hh_inc_med_adj', 'pop16_unemp_pct', 'pop_edu_nohs_pct')]), ses_df$crime_rate, alpha = 1)

best_lambda <- cv_model$lambda.min

# Conducting the Lasso fit 
lasso_fit = glmnet(data.matrix(ses_df[, c('pop_pov_pct', 'hh_inc_med_adj', 'pop16_unemp_pct', 'pop_edu_nohs_pct')]), ses_df$crime_rate, alpha = 1, lambda = best_lambda)
